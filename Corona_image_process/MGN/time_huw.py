...
def anytim2cal(date_in0, tai=False, form=11, date_only=False, msec=False):
    ...
    for imain in range(n):
        ...
        if isinstance(date_in[imain], str) == False:
            if np.isnan(date_in[imain]):
                continue
            tai = True

        if tai:  # user has supplied date_in as TAI (this is the IDL/Solarsoft TAI, or seconds since 1957/12/31 23:59:51)
            date = parse_time(date_in[imain], scale='tai', format='tai_seconds')  # convert IDL tai to date object
        else:
            date = parse_time(
                date_in[imain])  # assumes user has supplied date as string, in one of the accepted formats

        date = date.iso

        if date_only:  # user requires only the date, without time
