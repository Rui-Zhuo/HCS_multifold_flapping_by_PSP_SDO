...
def tikhonov_opt_tomo(d):
    ...
    c = [np.sum(np.squeeze(d["sph"][:, :, isph]) * dens * d["sinlat"]) * d["dlon"] * d["dlat"] for isph in
         np.arange(0, d["nsph"])]
...
def tomo_make_geom(d, large=True):
    ...
    y, x, z = coord.spherical2cartesian(rsc, lonsc,
                                        latsc)  # y,x,z order because position angle (lonsc) measured from north
    z = dist - z
...
def tomo_prep_sph(geom, nlon=540, nlat=270, norder=22):
    ...
    for l in range(0, nl + 1):
        for m in range(-l, l + 1):
            ...
            cenx_sph[:, :, isph] = func.make_spher_harm(nl, geom["lon"][:, :, xcen], geom["lat"][:, :, xcen],
                                                        userlm=[l, m]) * geom["geomult"][:, :, xcen] / dlon
            isph += 1
